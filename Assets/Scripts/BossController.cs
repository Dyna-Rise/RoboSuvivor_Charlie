...
public class BossController : MonoBehaviour
{
    ...
    //<<< パブリック変数 >>>
    public int bossHP = 5;                          // ヒットポイント
    public GameObject body;                         // 自身のボディ(点滅対象)
    public float closeRange = 3f;                   // プレイヤーとの距離が遠いと判断する距離(近接攻撃をする距離)
    public float barrierDeploymentDistance = 5f;    // パリアを展開させるプレイヤーとの距離 
    public float barrierEffectiveTime = 3f;         // バリアの有効時間
    ...
    bool longRangeAttacks = false;      // 遠距離攻撃実行中フラグ(false=停止中/true=実行中)
...
//        barrierPrefab.SetActive(false);
    ...
    private void FixedUpdate()
    {
        ...
        {
            // 長距離攻撃実行中でなければ近距離攻撃を行う
            if (!longRangeAttacks)
            {
                // パリア展開中でなければバリアを展開する
                if (!barrierDeployment)
                {
                    // 攻撃中フラグをセットする
                    isAttacking = true;                    // 攻撃中かどうか
                    // バリア展開フラグをセットする
                    barrierDeployment = true;    // バリアを展開しているか(false=消去中/true=展開中)
                    // バリア展開コルーチンを呼び出す
                    Debug.Log($"longDistance={closeRange} playerDistance={playerDistance}");
    ...
    IEnumerator Barrier()
    {
        ...
        // バリアプレアブを生成する
        GameObject barrier = Instantiate(bulletPrefab, transform.position, Quaternion.identity); // 新しい回転を適用

        Vector3 barrierSize = barrier.transform.localScale;
        barrierPrefab.transform.localScale = new Vector3(barrierSize.x / 10.0f, barrierSize.y / 10.0f, barrierSize.z / 10.0f);

        //<<< バリアを表示する >>>
        // バリアが生成されて範囲が少しずつ広がるイメージ
        barrier.transform.localScale = Vector3.Lerp(barrier.transform.localScale, barrierSize, Time.deltaTime);

        while (barrierPrefab.transform.localScale.x < barrierSize.x)
        {
...
//        Destroy(barrier);
        ...
        // パリア展開中フラグをクリアする
        barrierDeployment = false;    // バリアを展開しているか(false=消去中/true=展開中)
    ...
    IEnumerator Tackle()
    {
        ...
        Debug.Log($"barrierSize={barrierSize}");

        // プレイヤーの位置を取得する
        Vector3 playerPosition = player.transform.position;

        //        while (Vector3.Distance(transform.position, playerPosition) > stopDistance)
        while (Vector3.Distance(transform.position, playerPosition) > barrierSize.x * 0.8)
        {
            // 対象への方向
        ...
        // 遠距離攻撃実行中フラグをクリアする
        longRangeAttacks = false;      // 遠距離攻撃実行中フラグ(false=停止中/true=実行中)
...
//        Transform gate = transform.Find("Gate");
    ...
    private void OnTriggerEnter(Collider collision)
    {
        ...
        else if (collision.gameObject.CompareTag("PlsyerSword"))
        {
            playerSword = true;
        }
        else
